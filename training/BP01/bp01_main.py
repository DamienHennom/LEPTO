...
def bp01_model(train,test,prj_info,TMP=1234):
    """Training pilot
    ...
    best_experiment,y_test,X_test,W_test,best_lightgbm,data_results_tp = lightgbm_tp(train,prj_info,training.BP01.settings.bp01_param)
...
def bp01_scoring(model,data,input_prj):
    """Scoring bp01
...
def lightgbm_tp(train,prj_info,setting):
    ...
    while True:
        ...
        lightgbm = lgb.train(
            ...
            verbose_eval = False)
        #Predict
        pred_train = lightgbm.predict(X_train, num_iteration=lightgbm.best_iteration)
        pred_test = lightgbm.predict(X_test, num_iteration=lightgbm.best_iteration)
        ...
        #Metric
        metric_train = error_metric(y_train,pred_train,W_train,prj_info['METRIC'])
